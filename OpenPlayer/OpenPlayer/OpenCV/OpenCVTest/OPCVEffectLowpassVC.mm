//
//  OPCVEffectLowpassVC.m
//  OpenPlayer
//
//  Created by 王宏鹤 on 2025/4/20.
//
...
@interface OPCVEffectLowpassVC ()

@end

@implementation OPCVEffectLowpassVC
...
- (cv::Mat)effect:(cv::Mat&)mat {
    ...
    cv::Mat filter = cv::Mat::zeros(channelPadded.size(), CV_32F);
    for(int i=0; i<100; i++) {
        for(int j=0; j<100; j++) {
