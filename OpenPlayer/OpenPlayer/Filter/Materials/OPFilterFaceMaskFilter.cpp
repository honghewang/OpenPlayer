...
void OPFilterFaceMaskFilter::renderFaceMask(std::shared_ptr<OPFilterRenderFilterInfoCenter> info) {
    std::vector<GLfloat> vec;
    std::vector<dlib::full_object_detection> &facePoints = info->aiInfo->facePoints;
    for (auto &shape : facePoints) {
        for (int i = 1; i < 15; i++) {
            vec.push_back(1.0 * shape.part(28).x() / info->width * 2 - 1);
            vec.push_back(1.0 * shape.part(28).y() / info->height * 2 - 1);
            
            vec.push_back(1.0 * shape.part(i).x() / info->width * 2 - 1);
            vec.push_back(1.0 * shape.part(i).y() / info->height * 2 - 1);
            
            vec.push_back(1.0 * shape.part(i+1).x() / info->width * 2 - 1);
            vec.push_back(1.0 * shape.part(i+1).y() / info->height * 2 - 1);
        }
    }
    if (vec.size() == 0) {
        return;
